...
for name, (a, b) in distributions.items():
    mean = a / (a + b)
    print(f"  {name:25s}: {mean:.4f}  (α={a}, β={b})")
...
ax1.set_xticklabels(short, fontsize=TICK_FONT_SIZE, color=color_palette["text"], rotation=30, ha='right')
ax1.set_yticklabels(short, fontsize=TICK_FONT_SIZE, color=color_palette["text"])
...
for i in range(n):
    for j in range(n):
        ...
        ax1.text(j, i, f"{val:.3f}", ha='center', va='center', fontsize=13, color=text_color, weight='bold')
...
ax2.set_xticklabels(short, fontsize=TICK_FONT_SIZE, color=color_palette["text"], rotation=30, ha='right')
ax2.set_yticklabels(short, fontsize=TICK_FONT_SIZE, color=color_palette["text"])
...
for i in range(n):
    for j in range(n):
        ...
        ax2.text(j, i, f"{val:+.4f}", ha='center', va='center', fontsize=13, color=text_color, weight='bold')
...
plt.tight_layout()
plt.savefig(
    'visualizations/beta_comparison_2.png',
    dpi=300,
    bbox_inches='tight',
    facecolor=fig.get_facecolor(),
)
plt.close()
print("Saved visualizations/beta_comparison_2.png")
